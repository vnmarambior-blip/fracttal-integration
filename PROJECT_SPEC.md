...
Para los 3 tests que importan `parse_equipment_snapshot_xml` (`test_mydevelon_fracttal.py:6`, `test_mydevelon_single.py:4`, `test_mydevelon_sql.py:4`): primero dictaminar la intención real de cada test. Esos tests prueban snapshot individual, no flota. Opciones excluyentes:

* (a) implementar `parse_equipment_snapshot_xml` real con TDD (es funcionalidad faltante), o
* (b) eliminar los tests si el snapshot individual ya no es un caso soportado.
...
El evento canónico de una actualización verificada es la fila de intención en `VERIFIED`. Se elimina el segundo insert `UPDATED` de `process_equipment` (`api.py:1933-1946`); mismo criterio para el path de error (`api.py:1902-1912`). Toda fila de auditoría conserva su `idempotency_key` cuando corresponda. Sin razón explícita y modelo que diferencie ambos eventos, no hay segunda fila.
...
Lectura sin fecha válida → estado revisión/error auditado (Regla 6). Lectura antigua nunca actualiza lectura Fracttal más reciente. Decisión explícita y documentada sobre `MAX_SOURCE_AGE_HOURS=48` / `ALLOW_OLD_SOURCE_UPDATES`: se implementan en el flujo de escritura o se eliminan del código y de `.env.example`. Sin tercer estado.
...
Prohibido remapear `serial` automáticamente por coincidencia de `equipment_code` (`database.py:544-570`). Un conflicto `equipment_code` / `serial` / `machinery_id` detiene el remapeo y genera estado auditable. Sin sobrescritura silenciosa.
...
El patrón check-then-insert (`save_horometer_update`, `database.py:686-698`) usa `TRY/CATCH` de violación única → traducir a `IdempotencyConflictError` (patrón existente de `create_horometer_write_intent:904-914`), o centralizar toda escritura con key en la función de intención. Una carrera no genera duplicados ni aborta la corrida. `attempt_count` se incrementa por intento, nunca fijo en 1. El `return existing` del dedup define si actualiza mensaje/estado o ignora (documentado, no silencioso).
...
Importar un módulo no debe: ejecutar POST a APIs, abrir conexiones reales, imprimir secretos, operar destructivamente ni modificar la BD. Todo script ejecutable bajo `if __name__ == "__main__":` explícito (alcanza a `oauth.py:13`, `check_machinery_serials.py:20`, `inspect_machinery.py:12`, `setup_telemetry_sync.py:12`, `test_sql.py:4`, `test_env.py:6`). Higiene de archivos: `cleanup_mydevelon_config.p` se renombra a `.py` con guarda o se elimina; `get_equipment_by_serial()` se renombra sin paréntesis o se elimina por duplicar `api.py:98`.
...
- Autenticación: OAuth2 `grant_type=password` + `HTTPBasicAuth` sobre `/provider/token`. Token válido **2 horas**.
...
- **Ejecutor**: `run_komtrax_sync.py` (modo file/dry-run por defecto, cero red Komtrax; `--live` consulta la API real con cuota mínima).
- **Gate productivo**: con `SYNC_DRY_RUN=false`, el ejecutor exige `gates_closed()` antes de cualquier red; si el gate está abierto imprime `BLOQUEADO: Regla 0 / H1 abiertos; solo dry-run permitido.` y sale con código 2. El gate es `KOMTRAX_SYNC_GATES_OK == "1"`, fijado manualmente solo tras verificar el cierre de Regla 0 (cero huérfanas) y H1. El modo dry-run (default) nunca consulta el gate.
